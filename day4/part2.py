...
if __name__ == "__main__":
    ...
    cardCount = [1] * (len(lines) + 1) #ingnoring index zero to make the math easier
    cardCount[0] = 0
    currentCard = 1
    #while(cardCount[currentCard] > 0):
    for currentCard in range(1, len(lines) + 1):
        cardNum, list1, list2 = readCard(lines[currentCard - 1])
        matches = numberMatching(set(list1), set(list2))
        currentCardCount = cardCount[currentCard]
        for i in range(1, matches + 1):
            cardCount[i + currentCard] += currentCardCount
        currentCard += 1
    print(sum(cardCount))
