...
def get_most_common_key(mydict: dict):
    maxVal = max(mydict.values())
    for key, val in mydict.items():
        if maxVal == val:
            return key
    print(f"There was en error getting the most common key from {mydict}")
    return None

def encodeHand(handString: str):
    ms = multiset.Multiset(handString)
    if 'J' in set(ms) and ms['J'] != 5: # if there are jokers, count them as the most prevalent card instead
        numJokers = ms['J']
        del ms['J']
        ms[get_most_common_key(ms)] += numJokers
